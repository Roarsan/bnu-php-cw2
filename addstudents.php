    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($_POST['studentid']) || empty($_POST['password']) 
            ...
            $data['content'] .= "<h3 class='fw-bold text-danger'>Please fill in ALL required fields to continue</h3>" .
                "<input type='button' class='btn btn-outline-primary' value='Return to Form' onclick='window.location.href=\"addstudent.php\"'><br/>";
        } else {
            ...
            if (!empty($image)) {
                ...
                $sql = "INSERT INTO student (studentid, password, dob, firstname, lastname, house, town, county, country, postcode)
                        VALUES ('{$_POST['studentid']}', '{$hashed_password}', '{$_POST['dob']}', '{$_POST['firstname']}', '{$_POST['lastname']}', '{$_POST['house']}', '{$_POST['town']}', '{$_POST['county']}', '{$_POST['country']}', '{$_POST['postcode']}')";
            }
            ...
            // Display message
            $data['content'] .= "<h3 class='fw-bold text-primary'>Student record has been added! Press return to form to add another entry</h3>" .
                "<input type='button' class='btn btn-outline-primary' value='Return to Form' onclick='window.location.href=\"addstudent.php\"'><br/>";
        }
    } else 
    {
